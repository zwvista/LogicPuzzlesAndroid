#!/usr/bin/env python3
"""
递归检查指定子目录下所有Kotlin文件，查找包含字母数字+Game.的字符串（如xxxGame.），
排除文件名以相同Game名称开头的文件
"""
...
SEARCH_SUBDIR = 'app/src/main/java/com/zwstudio/logicpuzzlesandroid/puzzles'

# 文件扩展名配置
FILE_EXTENSION = '.kt'  # Kotlin文件扩展名

# 过滤选项
IGNORE_IMPORT = True     # 忽略import语句
IGNORE_COMMENTS = False  # 检查注释
IGNORE_COMPANION = False # 检查Companion引用

def find_game_pattern_in_files(root_dir, search_subdir, file_extension):
    """
    在指定子目录下递归查找所有Kotlin文件中包含"字母数字+Game."的字符串，
    排除文件名以相同Game名称开头的文件
    
    Args:
        root_dir: 要搜索的根目录路径
        search_subdir: 要搜索的子目录路径（相对于root_dir）
        file_extension: 文件扩展名（如 '.kt' 或 '.java'）
    """
        ...
        for file in files:
            ...
            try:
                ...
                for line_num, line in enumerate(lines, 1):
                    # 只过滤import语句
                    if IGNORE_IMPORT and line.strip().startswith('import '):
                        continue
...
def print_results(results):
    ...
    """
    if not results:
        print("未找到任何包含'字母数字+Game.'模式的Kotlin文件")
        return
...
def main():
    ...
    print(f"过滤条件: 只忽略import语句")
    print("-" * 80)
    
    # 执行搜索
    results = find_game_pattern_in_files(root_dir, SEARCH_SUBDIR, FILE_EXTENSION)
