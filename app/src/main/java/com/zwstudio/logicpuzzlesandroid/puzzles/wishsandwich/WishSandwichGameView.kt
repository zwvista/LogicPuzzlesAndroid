...
class WishSandwichGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as WishSandwichGameActivity
...
//        canvas.drawColor(Color.BLACK);
        for (r in 0 until rows)
            for (c in 0 until cols) {
                ...
                when (val o = game.getObject(p)) {
                    ...
                    is WishSandwichForbiddenObject ->
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !game.isSolved) {
            ...
            val move = WishSandwichGameMove(Position(row, col))
