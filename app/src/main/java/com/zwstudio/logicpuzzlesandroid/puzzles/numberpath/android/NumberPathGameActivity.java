...
public class NumberPathGameActivity extends GameGameActivity<NumberPathGame, NumberPathDocument, NumberPathGameMove, NumberPathGameState> {
    @Bean
    protected NumberPathDocument document;
    public NumberPathDocument doc() {return document;}

    protected NumberPathGameView gameView;
    protected View getGameView() {return gameView;}

    @AfterViews
    protected void init() {
        gameView = new NumberPathGameView(this);
        super.init();
    }
    ...
    protected void startGame() {
        ...
        game = new NumberPathGame(level.layout, this, doc());
        try {
            // restore game state
            for (MoveProgress rec : doc().moveProgress()) {
                NumberPathGameMove move = doc().loadMove(rec);
                game.setObject(move);
            }
