...
public class PataGame extends CellsGame<PataGame, PataGameMove, PataGameState> {
    ...
    public PataGame(List<String> layout, GameInterface<PataGame, PataGameMove, PataGameState> gi, GameDocumentInterface gdi) {
        super(gi, gdi);
        ...
        PataGameState state = new PataGameState(this);
        states.add(state);
        levelInitilized(state);
    }

    private boolean changeObject(PataGameMove move, F2<PataGameState, PataGameMove, Boolean> f) {
        if (canRedo()) {
            states.subList(stateIndex + 1, states.size()).clear();
            moves.subList(stateIndex, states.size()).clear();
        }
        PataGameState state = cloner.deepClone(state());
    ...
    public PataObject getObject(int row, int col) {
