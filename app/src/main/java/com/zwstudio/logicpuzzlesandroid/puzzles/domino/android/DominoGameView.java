...
public class DominoGameView extends CellsGameView {
    ...
    private DominoGame game() {return activity().game;}
    ...
    public DominoGameView(Context context) {
        super(context);
        init(null, 0);
    }

    public DominoGameView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public DominoGameView(Context context, AttributeSet attrs, int defStyle) {
...
//        canvas.drawColor(Color.BLACK);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN && !game().isSolved()) {
            ...
            DominoGameMove move = new DominoGameMove() {{
