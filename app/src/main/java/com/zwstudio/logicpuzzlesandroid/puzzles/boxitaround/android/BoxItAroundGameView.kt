...
class BoxItAroundGameView : CellsGameView {
    private fun activity(): BoxItAroundGameActivity {
        return context as BoxItAroundGameActivity
    }

    private fun game(): BoxItAroundGame? {
...
//        canvas.drawColor(Color.BLACK);
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !game()!!.isSolved) {
            ...
            val move: BoxItAroundGameMove = object : BoxItAroundGameMove() {
                init {
                    p = Position(row, col)
