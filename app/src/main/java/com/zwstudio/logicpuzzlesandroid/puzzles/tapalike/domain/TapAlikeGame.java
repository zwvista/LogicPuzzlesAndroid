...
public class TapAlikeGame extends CellsGame<TapAlikeGame, TapAlikeGameMove, TapAlikeGameState> {
    ...
    public TapAlikeGame(List<String> layout, GameInterface<TapAlikeGame, TapAlikeGameMove, TapAlikeGameState> gi, GameDocumentInterface gdi) {
        super(gi, gdi);
        ...
        TapAlikeGameState state = new TapAlikeGameState(this);
        states.add(state);
        levelInitilized(state);
    }

    private boolean changeObject(TapAlikeGameMove move, F2<TapAlikeGameState, TapAlikeGameMove, Boolean> f) {
        if (canRedo()) {
            states.subList(stateIndex + 1, states.size()).clear();
            moves.subList(stateIndex, states.size()).clear();
        }
        TapAlikeGameState state = cloner.deepClone(state());
    ...
    public TapAlikeObject getObject(int row, int col) {
