...
public class MathraxGame extends CellsGame<MathraxGame, MathraxGameMove, MathraxGameState> {
    ...
    public MathraxGame(List<String> layout, GameInterface<MathraxGame, MathraxGameMove, MathraxGameState> gi, GameDocumentInterface gdi) {
        ...
        for (int r = 0; r < rows(); r++)
            for (int c = 0; c < cols(); c++) {
                Position p = new Position(r, c);
                for (int i = 0; i < 4; i++)
                    if (dots.get(p.add(MathraxGame.offset2[i]), MathraxGame.dirs[i]) != GridLineObject.Line)
                        g.connectNode(pos2node.get(p), pos2node.get(p.add(MathraxGame.offset[i * 2])));
            }
        while (!rng.isEmpty()) {
    ...
    public MathraxObject getObject(Position p) {
        return state().get(p);
    }

    public MathraxObject getObject(int row, int col) {
