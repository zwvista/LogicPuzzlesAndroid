...
class SnakeominoGameState(game: SnakeominoGame) : CellsGameState<SnakeominoGame, SnakeominoGameMove, SnakeominoGameState>(game) {
    ...
    override fun setObject(move: SnakeominoGameMove): GameOperationType {
        ...
        val p2 = p1 + SnakeominoGame.offset[dir]
    ...
    */
    private fun updateIsSolved() {
        isSolved = true
        val rects = mutableListOf<SnakeominoRect>()
        ...
        for (r in 0 until rows - 1)
            for (c in 0 until cols - 1) {
                ...
                if (game[p] != SnakeominoObject.Hedge) {
        ...
        for ((p, node) in pos2node)
            for (i in 0 until 4) {
                if (this[p + SnakeominoGame.offset2[i], SnakeominoGame.dirs[i]] == GridLineObject.Line) continue
                val node2 = pos2node[p + SnakeominoGame.offset[i]]
                if (node2 != null)
        ...
        while (pos2node.isNotEmpty()) {
            ...
            if (s == HintState.Complete) {
                val n = rects.size
                rects.add(SnakeominoRect(area, rs, cs))
