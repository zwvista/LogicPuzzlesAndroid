...
class BoxItAgainGameState(game: BoxItAgainGame) : CellsGameState<BoxItAgainGame, BoxItAgainGameMove, BoxItAgainGameState>(game) {
    ...
    fun setObject(move: BoxItAgainGameMove?): Boolean {
        ...
        val p2 = p1!!.add(BoxItAgainGame.Companion.offset.get(dir))
        get(p1)!![dir] = move.obj
    ...
    */
    private fun updateIsSolved() {
        ...
        for (r in 0 until rows() - 1) for (c in 0 until cols() - 1) {
            val p = Position(r, c)
            for (i in 0..3) if (get(p.add(BoxItAgainGame.Companion.offset2.get(i)))!![BoxItAgainGame.Companion.dirs.get(i)] != GridLineObject.Line) g.connectNode(pos2node[p], pos2node[p.add(BoxItAgainGame.Companion.offset.get(i))])
        }
