...
public class RobotFencesGame extends CellsGame<RobotFencesGame, RobotFencesGameMove, RobotFencesGameState> {
    ...
    public RobotFencesGame(List<String> layout, GameInterface<RobotFencesGame, RobotFencesGameMove, RobotFencesGameState> gi, GameDocumentInterface gdi) {
        super(gi, gdi);
        ...
        for (int r = 0; r < rows(); r++)
            for (int c = 0; c < cols(); c++) {
                Position p = new Position(r, c);
                for (int i = 0; i < 4; i++)
                    if (dots.get(p.add(RobotFencesGame.offset2[i]), RobotFencesGame.dirs[i]) != GridLineObject.Line)
                        g.connectNode(pos2node.get(p), pos2node.get(p.add(RobotFencesGame.offset[i])));
            }
        while (!rng.isEmpty()) {
        ...
        RobotFencesGameState state = new RobotFencesGameState(this);
        states.add(state);
        levelInitilized(state);
    }

    private boolean changeObject(RobotFencesGameMove move, F2<RobotFencesGameState, RobotFencesGameMove, Boolean> f) {
        if (canRedo()) {
            states.subList(stateIndex + 1, states.size()).clear();
            moves.subList(stateIndex, states.size()).clear();
        }
        RobotFencesGameState state = cloner.deepClone(state());
