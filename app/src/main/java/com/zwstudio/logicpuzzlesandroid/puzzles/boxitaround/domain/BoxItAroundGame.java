...
public class BoxItAroundGame extends CellsGame<BoxItAroundGame, BoxItAroundGameMove, BoxItAroundGameState> {
    ...
    public BoxItAroundGame(List<String> layout, GameInterface<BoxItAroundGame, BoxItAroundGameMove, BoxItAroundGameState> gi, GameDocumentInterface gdi) {
        super(gi, gdi);
        ...
        BoxItAroundGameState state = new BoxItAroundGameState(this);
        states.add(state);
        levelInitilized(state);
    }

    private boolean changeObject(BoxItAroundGameMove move, F2<BoxItAroundGameState, BoxItAroundGameMove, Boolean> f) {
        if (canRedo()) {
            states.subList(stateIndex + 1, states.size()).clear();
            moves.subList(stateIndex, states.size()).clear();
        }
        BoxItAroundGameState state = cloner.deepClone(state());
    ...
    public boolean switchObject(BoxItAroundGameMove move) {
        return changeObject(move, BoxItAroundGameState::switchObject);
    }

    public boolean setObject(BoxItAroundGameMove move) {
        return changeObject(move, BoxItAroundGameState::setObject);
    }
