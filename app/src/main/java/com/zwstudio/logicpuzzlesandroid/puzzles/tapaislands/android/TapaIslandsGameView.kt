...
//        canvas.drawColor(Color.BLACK);
        for (r in 0 until rows()) for (c in 0 until cols()) {
            ...
            val o: TapaIslandsObject = game().getObject(r, c)
            if (o is TapaIslandsWallObject) {
                val o2: TapaIslandsWallObject = o as TapaIslandsWallObject
            ...
            } else if (o is TapaIslandsMarkerObject) canvas.drawArc(cwc2(c) - 20.toFloat(), chr2(r) - 20.toFloat(), cwc2(c) + 20.toFloat(), chr2(r) + 20.toFloat(), 0f, 360f, true, wallPaint)
        }
    }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.getAction() == MotionEvent.ACTION_DOWN && !game().isSolved()) {
            ...
            val move: TapaIslandsGameMove = object : TapaIslandsGameMove() {
                init {
                    p = Position(row, col)
