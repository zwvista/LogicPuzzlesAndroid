...
class OnlyStraightsGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as OnlyStraightsGameActivity
    ...
    init {
        ...
        dTown = fromImageToDrawable("images/lodge.png")
    }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> if (p != pLastMove) {
                val n = OnlyStraightsGame.offset.indexOfFirst { it == p - pLastMove!! }
                if (n != -1) {
                    val move = OnlyStraightsGameMove(pLastMove!!, n)
            ...
            MotionEvent.ACTION_UP -> {
                if (p == pLastDown) {
                    ...
                    val dx2 = abs(dx)
                    val dy2 = abs(dy)
                    val move = OnlyStraightsGameMove(Position(row, col), if (dx2 <= dy2) if (dy > 0) 2 else 0 else if (dy2 <= dx2) if (dx > 0) 1 else 3 else 0)
                    game.setObject(move)
                }
                run {
                    pLastMove = null
