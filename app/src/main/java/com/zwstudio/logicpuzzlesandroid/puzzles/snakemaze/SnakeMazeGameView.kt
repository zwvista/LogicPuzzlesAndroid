...
class SnakeMazeGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    ...
    private val gestureDetector = GestureDetector(context, object : GestureDetector.SimpleOnGestureListener() {
        override fun onLongPress(e: MotionEvent) {
            ...
            if (game.setObject(SnakeMazeGameMove(p, SnakeMazeGame.PUZ_DIR_SQUARE)))
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> if (pLastMove != null && p != pLastMove) {
                val n = SnakeMazeGame.offset.indexOfFirst { it == p - pLastMove!! }
                if (n != -1) {
                    val move = SnakeMazeGameMove(pLastMove!!, n)
            ...
            MotionEvent.ACTION_UP -> {
                if (p == pLastDown) {
                    ...
                    val dx2 = abs(dx)
                    val dy2 = abs(dy)
                    val move = SnakeMazeGameMove(Position(row, col), if (dx2 <= dy2) if (dy > 0) 2 else 0 else if (dx > 0) 1 else 3)
                    game.setObject(move)
                }
                run {
                    pLastMove = null
