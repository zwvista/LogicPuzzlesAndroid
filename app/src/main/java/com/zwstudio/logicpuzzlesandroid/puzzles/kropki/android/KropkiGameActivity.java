...
public class KropkiGameActivity extends GameGameActivity<KropkiGame, KropkiDocument, KropkiGameMove, KropkiGameState> {
    @Bean
    protected KropkiDocument document;
    public KropkiDocument doc() {return document;}

    protected KropkiGameView gameView;
    protected View getGameView() {return gameView;}

    @AfterViews
    protected void init() {
        gameView = new KropkiGameView(this);
        super.init();
    }
    ...
    protected void startGame() {
        ...
        game = new KropkiGame(level.layout, "1".equals(level.settings.get("Bordered")),this, doc());
        try {
            // restore game state
            for (MoveProgress rec : doc().moveProgress()) {
                KropkiGameMove move = doc().loadMove(rec);
                game.setObject(move);
            }
