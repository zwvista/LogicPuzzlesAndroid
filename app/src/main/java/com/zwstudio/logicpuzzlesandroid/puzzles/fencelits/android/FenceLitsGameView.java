...
public class FenceLitsGameView extends CellsGameView {
    ...
    public FenceLitsGameView(Context context) {
        super(context);
        init(null, 0);
    }

    public FenceLitsGameView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public FenceLitsGameView(Context context, AttributeSet attrs, int defStyle) {
...
//        canvas.drawColor(Color.BLACK);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN && !game().isSolved()) {
            ...
            FenceLitsGameMove move = new FenceLitsGameMove() {{
