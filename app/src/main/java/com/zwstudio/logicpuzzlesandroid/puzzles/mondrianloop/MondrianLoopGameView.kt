...
class MondrianLoopGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as MondrianLoopGameActivity
...
//        canvas.drawColor(Color.BLACK);
        for (r in 0 until rows)
            for (c in 0 until cols) {
                ...
                val p = Position(r, c)
                if (game.rectangles().any { it.contains(p) }) {
                ...
                val text = if (n == MondrianLoopGame.PUZ_UNKNOWN) "?" else n.toString()
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !game.isSolved) {
            ...
            val move = MondrianLoopGameMove(Position(row, col), if (yOffset >= -offset && yOffset <= offset) 1 else 2)
