...
class BoxItAroundGame(layout: List<String>, gi: GameInterface<BoxItAroundGame?, BoxItAroundGameMove?, BoxItAroundGameState?>?, gdi: GameDocumentInterface?) : CellsGame<BoxItAroundGame?, BoxItAroundGameMove?, BoxItAroundGameState?>(gi, gdi) {
    ...
    private fun changeObject(move: BoxItAroundGameMove?, f: F2<BoxItAroundGameState?, BoxItAroundGameMove?, Boolean>): Boolean {
        if (canRedo()) {
    ...
    fun switchObject(move: BoxItAroundGameMove?): Boolean {
        return changeObject(move, F2 { obj: BoxItAroundGameState?, move: BoxItAroundGameMove? -> obj!!.switchObject(move) })
    }

    fun setObject(move: BoxItAroundGameMove?): Boolean {
        return changeObject(move, F2 { obj: BoxItAroundGameState?, move: BoxItAroundGameMove? -> obj!!.setObject(move) })
    }
