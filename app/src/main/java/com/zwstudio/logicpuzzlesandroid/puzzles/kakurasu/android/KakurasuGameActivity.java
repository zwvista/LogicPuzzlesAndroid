...
public class KakurasuGameActivity extends GameGameActivity<KakurasuGame, KakurasuDocument, KakurasuGameMove, KakurasuGameState> {
    @Bean
    protected KakurasuDocument document;
    public KakurasuDocument doc() {return document;}

    protected KakurasuGameView gameView;
    protected View getGameView() {return gameView;}

    @AfterViews
    protected void init() {
        gameView = new KakurasuGameView(this);
        super.init();
    }
    ...
    protected void startGame() {
        ...
        game = new KakurasuGame(level.layout, this, doc());
        try {
            // restore game state
            for (MoveProgress rec : doc().moveProgress()) {
                KakurasuGameMove move = doc().loadMove(rec);
                game.setObject(move);
            }
