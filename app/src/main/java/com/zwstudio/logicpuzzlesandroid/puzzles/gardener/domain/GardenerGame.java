...
public class GardenerGame extends CellsGame<GardenerGame, GardenerGameMove, GardenerGameState> {
    ...
    public Map<Position, P2<Integer, Integer>> pos2hint = new HashMap<>();

    public GardenerGame(List<String> layout, GameInterface<GardenerGame, GardenerGameMove, GardenerGameState> gi, GameDocumentInterface gdi) {
        super(gi, gdi);
        ...
        for (int r = 0; r < rows() + 1; r++) {
            ...
            for (int c = 0; c < cols() + 1; c++) {
                ...
                if (c == cols()) break;
                ch = str.charAt(c * 2 + 1);
                if (ch != ' ') pos2hint.put(new Position(r, c), P.p(ch - '0', -1));
            }
        }
        ...
        for (int r = 0; r < rows(); r++)
            for (int c = 0; c < cols(); c++) {
                Position p = new Position(r, c);
                for (int i = 0; i < 4; i++)
                    if (dots.get(p.add(GardenerGame.offset2[i]), GardenerGame.dirs[i]) != GridLineObject.Line)
                        g.connectNode(pos2node.get(p), pos2node.get(p.add(GardenerGame.offset[i])));
            }
        while (!rng.isEmpty()) {
        ...
        for (Map.Entry<Position, P2<Integer, Integer>> entry : pos2hint.entrySet()) {
            Position p = entry.getKey();
            int n = entry.getValue()._1();
            pos2hint.put(p, P.p(n, pos2area.get(p)));
        }

        GardenerGameState state = new GardenerGameState(this);
        states.add(state);
        levelInitilized(state);
    }

    private boolean changeObject(GardenerGameMove move, F2<GardenerGameState, GardenerGameMove, Boolean> f) {
        if (canRedo()) {
            states.subList(stateIndex + 1, states.size()).clear();
            moves.subList(stateIndex, states.size()).clear();
        }
        GardenerGameState state = cloner.deepClone(state());
    ...
    public GardenerObject getObject(int row, int col) {
