...
class FenceLitsGameView : CellsGameView {
    private fun activity(): FenceLitsGameActivity {
        return context as FenceLitsGameActivity
    }

    private fun game(): FenceLitsGame? {
...
//        canvas.drawColor(Color.BLACK);
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !game()!!.isSolved) {
            ...
            val move: FenceLitsGameMove = object : FenceLitsGameMove() {
                init {
                    p = Position(row, col)
