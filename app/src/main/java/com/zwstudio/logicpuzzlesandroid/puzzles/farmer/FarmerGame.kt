...
class FarmerGame(layout: List<String>, gi: GameInterface<FarmerGame, FarmerGameMove, FarmerGameState>, gdi: GameDocumentInterface) : CellsGame<FarmerGame, FarmerGameMove, FarmerGameState>(gi, gdi) {
    companion object {
    ...
    val dots: GridDots
    val objArray: Array<FarmerObject>

    operator fun get(row: Int, col: Int) = objArray[row * cols + col]
    operator fun get(p: Position) = this[p.row, p.col]
    operator fun set(row: Int, col: Int, obj: FarmerObject) {objArray[row * cols + col] = obj}
    ...
    init {
        ...
        objArray = Array(rows * cols) { FarmerObject.Empty }
        for (r in 0 until rows + 1) {
            ...
            for (c in 0 until cols + 1) {
                ...
                set(Position(r, c), FarmerObject.entries[n])
            }
        }
        ...
        val state = FarmerGameState(this)
