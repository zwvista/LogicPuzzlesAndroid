...
class FunnyNumbersGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as FunnyNumbersGameActivity
...
//        canvas.drawColor(Color.BLACK);
        for (r in 0 until rows)
            for (c in 0 until cols) {
                ...
                when (val o = game.getObject(p)) {
                    is FunnyNumbersForbiddenObject ->
                        canvas.drawArc(cwc2(c) - 20.toFloat(), chr2(r) - 20.toFloat(), cwc2(c) + 20.toFloat(), chr2(r) + 20.toFloat(), 0f, 360f, true, forbiddenPaint)
                    is FunnyNumbersMarkerObject ->
                        canvas.drawArc(cwc2(c) - 20.toFloat(), chr2(r) - 20.toFloat(), cwc2(c) + 20.toFloat(), chr2(r) + 20.toFloat(), 0f, 360f, true, markerPaint)
                    is FunnyNumbersWaterObject -> {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !game.isSolved) {
            ...
            val move = FunnyNumbersGameMove(Position(row, col))
