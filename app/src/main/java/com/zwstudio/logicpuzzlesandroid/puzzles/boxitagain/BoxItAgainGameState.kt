...
class BoxItAgainGameState(game: BoxItAgainGame) : CellsGameState<BoxItAgainGame, BoxItAgainGameMove, BoxItAgainGameState>(game) {
    ...
    override fun setObject(move: BoxItAgainGameMove): GameOperationType {
        val dir = move.dir
        val dir2 = (dir + 2) % 4
        val p1 = move.p
        val p2 = p1 + BoxItAgainGame.offset[dir]
    ...
    override fun switchObject(move: BoxItAgainGameMove): GameOperationType {
    ...
    private fun updateIsSolved() {
        ...
        for (r in 0..<rows - 1)
            for (c in 0..<cols - 1) {
                val p = Position(r, c)
                for (i in 0..<4)
                    if (this[p + BoxItAgainGame.offset2[i]][BoxItAgainGame.dirs[i]] != GridLineObject.Line)
                        g.connectNode(pos2node[p]!!, pos2node[p + BoxItAgainGame.offset[i]]!!)
            }
