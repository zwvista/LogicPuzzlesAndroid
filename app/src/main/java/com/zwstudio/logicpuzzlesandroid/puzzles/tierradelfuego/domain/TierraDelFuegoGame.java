    ...
    public TierraDelFuegoGame(List<String> layout, GameInterface<TierraDelFuegoGame, TierraDelFuegoGameMove, TierraDelFuegoGameState> gi, GameDocumentInterface gdi) {
        ...
        for (int r = 0; r < rows(); r++)
            for (int c = 0; c < cols(); c++) {
                Position p = new Position(r, c);
                for (int i = 0; i < 4; i++)
                    if (dots.get(p.add(TierraDelFuegoGame.offset2[i]), TierraDelFuegoGame.dirs[i]) != GridLineObject.Line)
                        g.connectNode(pos2node.get(p), pos2node.get(p.add(TierraDelFuegoGame.offset[i * 2])));
            }
        while (!rng.isEmpty()) {
