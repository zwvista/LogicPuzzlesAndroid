...
class BoxItUpGameState(game: BoxItUpGame) : CellsGameState<BoxItUpGame?, BoxItUpGameMove?, BoxItUpGameState?>(game) {
    ...
    fun setObject(move: BoxItUpGameMove?): Boolean {
        ...
        val p2 = p1!!.add(BoxItUpGame.Companion.offset.get(dir))
        get(p1)!![dir] = move.obj
    ...
    */
    private fun updateIsSolved() {
        ...
        for (r in 0 until rows() - 1) for (c in 0 until cols() - 1) {
            val p = Position(r, c)
            for (i in 0..3) if (get(p.add(BoxItUpGame.Companion.offset2.get(i)))!![BoxItUpGame.Companion.dirs.get(i)] != GridLineObject.Line) g.connectNode(pos2node[p], pos2node[p.add(BoxItUpGame.Companion.offset.get(i))])
        }
