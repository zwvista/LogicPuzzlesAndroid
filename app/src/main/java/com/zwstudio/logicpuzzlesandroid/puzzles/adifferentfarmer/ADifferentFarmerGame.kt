...
class ADifferentFarmerGame(layout: List<String>, gi: GameInterface<ADifferentFarmerGame, ADifferentFarmerGameMove, ADifferentFarmerGameState>, gdi: GameDocumentInterface) : CellsGame<ADifferentFarmerGame, ADifferentFarmerGameMove, ADifferentFarmerGameState>(gi, gdi) {
    companion object {
    ...
    operator fun set(row: Int, col: Int, obj: ADifferentFarmerObject) {objArray[row * cols + col] = obj}
    ...
    init {
        ...
        objArray = Array(rows * cols) { ADifferentFarmerObject.Empty }
        for (r in 0..<rows + 1) {
            var str = layout[r * 2]
            for (c in 0..<cols) {
                val ch = str[c * 2 + 1]
            ...
            for (c in 0..<cols + 1) {
                ...
                set(Position(r, c), ADifferentFarmerObject.entries[n])
            }
        }
        ...
        val state = ADifferentFarmerGameState(this)
