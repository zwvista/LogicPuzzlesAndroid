...
public class WallsGameActivity extends GameGameActivity<WallsGame, WallsDocument, WallsGameMove, WallsGameState> {
    @Bean
    protected WallsDocument document;
    public WallsDocument doc() {return document;}

    protected WallsGameView gameView;
    protected View getGameView() {return gameView;}

    @AfterViews
    protected void init() {
        gameView = new WallsGameView(this);
        super.init();
    }
    ...
    protected void startGame() {
        ...
        game = new WallsGame(level.layout, this, doc());
        try {
            // restore game state
            for (MoveProgress rec : doc().moveProgress()) {
                WallsGameMove move = doc().loadMove(rec);
                game.setObject(move);
            }
