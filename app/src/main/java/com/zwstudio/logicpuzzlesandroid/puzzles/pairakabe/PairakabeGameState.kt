...
class PairakabeGameState(game: PairakabeGame) : CellsGameState<PairakabeGame, PairakabeGameMove, PairakabeGameState>(game) {
    ...
    override fun setObject(move: PairakabeGameMove): GameChangeType {
        val p = move.p
        val objOld = this[p]
        val objNew = move.obj
        if (objOld is PairakabeHintObject || objOld.toString() == objNew.toString()) return GameChangeType.None
        this[p] = objNew
        updateIsSolved()
        return GameChangeType.Level
    }

    override fun switchObject(move: PairakabeGameMove): GameChangeType {
