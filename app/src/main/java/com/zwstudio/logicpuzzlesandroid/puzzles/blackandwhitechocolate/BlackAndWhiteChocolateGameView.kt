...
class BlackAndWhiteChocolateGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as BlackAndWhiteChocolateGameActivity
...
//        canvas.drawColor(Color.BLACK);
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !game.isSolved) {
            ...
            val move = BlackAndWhiteChocolateGameMove(Position(row, col), if (yOffset >= -offset && yOffset <= offset) 1 else 2)
