...
class InbetweenNurikabeGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as InbetweenNurikabeGameActivity
...
//        canvas.drawColor(Color.BLACK);
        for (r in 0 until rows)
            for (c in 0 until cols) {
                ...
                when (game.getObject(p)) {
                    ...
                    InbetweenNurikabeObject.Wall ->
                        canvas.drawRect(cwc(c) + 4.toFloat(), chr(r) + 4.toFloat(), cwc(c + 1) - 4.toFloat(), chr(r + 1) - 4.toFloat(), wallPaint)
                    InbetweenNurikabeObject.Marker ->
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !game.isSolved) {
            ...
            val move = InbetweenNurikabeGameMove(Position(row, col))
