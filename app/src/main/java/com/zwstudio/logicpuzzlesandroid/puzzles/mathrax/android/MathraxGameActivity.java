...
public class MathraxGameActivity extends GameGameActivity<MathraxGame, MathraxDocument, MathraxGameMove, MathraxGameState> {
    @Bean
    protected MathraxDocument document;
    public MathraxDocument doc() {return document;}

    protected MathraxGameView gameView;
    protected View getGameView() {return gameView;}

    @AfterViews
    protected void init() {
        gameView = new MathraxGameView(this);
        super.init();
    }
    ...
    protected void startGame() {
        ...
        game = new MathraxGame(level.layout, this, doc());
        try {
            // restore game state
            for (MoveProgress rec : doc().moveProgress()) {
                MathraxGameMove move = doc().loadMove(rec);
                game.setObject(move);
            }
