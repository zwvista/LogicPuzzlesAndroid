...
class TapDifferentlyGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as TapDifferentlyGameActivity
...
//        canvas.drawColor(Color.BLACK);
        for (r in 0 until rows)
            for (c in 0 until cols) {
                ...
                when (val o = game.getObject(r, c)) {
                    is TapDifferentlyWallObject ->
                        canvas.drawRect(cwc(c) + 4.toFloat(), chr(r) + 4.toFloat(), cwc(c + 1) - 4.toFloat(), chr(r + 1) - 4.toFloat(), wallPaint)
                    is TapDifferentlyHintObject -> {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !game.isSolved) {
            ...
            val move = TapDifferentlyGameMove(Position(row, col))
