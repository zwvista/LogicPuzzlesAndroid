...
class TapAlikeGameState(game: TapAlikeGame) : CellsGameState<TapAlikeGame, TapAlikeGameMove, TapAlikeGameState>(game) {
    ...
    override fun setObject(move: TapAlikeGameMove): GameChangeType {
        val p = move.p
        val objOld = this[p]
        val objNew = move.obj
        if (objOld is TapAlikeHintObject || objOld == objNew) return GameChangeType.None
        this[p] = objNew
        updateIsSolved()
        return GameChangeType.Level
    }

    override fun switchObject(move: TapAlikeGameMove): GameChangeType {
