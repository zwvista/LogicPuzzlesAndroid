...
class TapDifferentlyGameState(game: TapDifferentlyGame) : CellsGameState<TapDifferentlyGame, TapDifferentlyGameMove, TapDifferentlyGameState>(game) {
    ...
    override fun setObject(move: TapDifferentlyGameMove): GameChangeType {
        val p = move.p
        val objOld = this[p]
        val objNew = move.obj
        if (objOld is TapDifferentlyHintObject || objOld == objNew) return GameChangeType.None
        this[p] = objNew
        updateIsSolved()
        return GameChangeType.Level
    }

    override fun switchObject(move: TapDifferentlyGameMove): GameChangeType {
