...
public class TapARowGame extends CellsGame<TapARowGame, TapARowGameMove, TapARowGameState> {
    ...
    public TapARowGame(List<String> layout, GameInterface<TapARowGame, TapARowGameMove, TapARowGameState> gi, GameDocumentInterface gdi) {
        super(gi, gdi);
        ...
        TapARowGameState state = new TapARowGameState(this);
        states.add(state);
        levelInitilized(state);
    }

    private boolean changeObject(TapARowGameMove move, F2<TapARowGameState, TapARowGameMove, Boolean> f) {
        if (canRedo()) {
            states.subList(stateIndex + 1, states.size()).clear();
            moves.subList(stateIndex, states.size()).clear();
        }
        TapARowGameState state = cloner.deepClone(state());
    ...
    public TapARowObject getObject(int row, int col) {
