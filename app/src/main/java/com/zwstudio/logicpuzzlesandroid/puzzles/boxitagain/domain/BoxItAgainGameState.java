...
public class BoxItAgainGameState extends CellsGameState<BoxItAgainGame, BoxItAgainGameMove, BoxItAgainGameState> {
    public GridLineObject[][] objArray;
    public Map<Position, HintState> pos2state = new HashMap<>();

    public BoxItAgainGameState(BoxItAgainGame game) {
        super(game);
    ...
    private void updateIsSolved() {
        ...
        for (int r = 0; r < rows() - 1; r++)
            for (int c = 0; c < cols() - 1; c++) {
                Position p = new Position(r, c);
                for (int i = 0; i < 4; i++)
                    if (get(p.add(BoxItAgainGame.offset2[i]))[BoxItAgainGame.dirs[i]] != GridLineObject.Line)
                        g.connectNode(pos2node.get(p), pos2node.get(p.add(BoxItAgainGame.offset[i])));
            }
