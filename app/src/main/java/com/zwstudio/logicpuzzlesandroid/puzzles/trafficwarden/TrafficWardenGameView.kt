...
class TrafficWardenGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as TrafficWardenGameActivity
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> if (p != pLastMove) {
                val n = TrafficWardenGame.offset.indexOfFirst { it == p - pLastMove!! }
                if (n != -1) {
                    val move = TrafficWardenGameMove(pLastMove!!, n)
