...
class TapDifferentlyGameState(game: TapDifferentlyGame) : CellsGameState<TapDifferentlyGame?, TapDifferentlyGameMove?, TapDifferentlyGameState?>(game) {
    var objArray: Array<TapDifferentlyObject?>
    operator fun get(row: Int, col: Int) = objArray[row * cols() + col]

    operator fun get(p: Position) = get(p!!.row, p.col)

    operator fun set(row: Int, col: Int, obj: TapDifferentlyObject?) {
        objArray[row * cols() + col] = obj
    }

    operator fun set(p: Position, obj: TapDifferentlyObject?) {
        set(p!!.row, p.col, obj)
    }

    fun setObject(move: TapDifferentlyGameMove): Boolean {
        val p: Position = move.p
        val objOld: TapDifferentlyObject? = get(p)
        val objNew: TapDifferentlyObject = move.obj
    ...
    fun switchObject(move: TapDifferentlyGameMove): Boolean {
        ...
        val f: F<TapDifferentlyObject, TapDifferentlyObject> = label@ F<TapDifferentlyObject, TapDifferentlyObject> { obj: TapDifferentlyObject? ->
            if (obj is TapDifferentlyEmptyObject) return@label if (markerOption == MarkerOptions.MarkerFirst) TapDifferentlyMarkerObject() else TapDifferentlyWallObject()
            if (obj is TapDifferentlyWallObject) return@label if (markerOption == MarkerOptions.MarkerLast) TapDifferentlyMarkerObject() else TapDifferentlyEmptyObject()
            if (obj is TapDifferentlyMarkerObject) return@label if (markerOption == MarkerOptions.MarkerFirst) TapDifferentlyWallObject() else TapDifferentlyEmptyObject()
            obj
        }
    ...
    */
    private fun updateIsSolved() {
        ...
        HashMap.fromMap<Position, List<Int>>(game.pos2hint).foreachDoEffect(Effect1<P2<Position, List<Int>>> { kv: P2<Position?, List<Int?>?> ->
            ...
            val o = TapDifferentlyHintObject()
            o.state = s
            set(p, o)
        ...
        for (c in 0 until cols()) {
            var n = 0
            for (r in 0 until rows()) if (get(r, c) is TapDifferentlyWallObject) n++
            nums.add(n)
        }
    ...
    init {
        objArray = arrayOfNulls<TapDifferentlyObject>(rows() * cols())
        for (i in objArray.indices) objArray[i] = TapDifferentlyEmptyObject()
        for (p in game.pos2hint.keys) set(p, TapDifferentlyHintObject())
    }
}