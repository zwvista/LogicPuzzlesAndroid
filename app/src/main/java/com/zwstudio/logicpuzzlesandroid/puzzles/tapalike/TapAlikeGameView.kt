...
class TapAlikeGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as TapAlikeGameActivity
        ...
        for (r in 0 until rows)
            for (c in 0 until cols) {
                ...
                when (val o = game.getObject(r, c)) {
                    is TapAlikeWallObject ->
                        canvas.drawRect(cwc(c) + 4.toFloat(), chr(r) + 4.toFloat(), cwc(c + 1) - 4.toFloat(), chr(r + 1) - 4.toFloat(), wallPaint)
                    is TapAlikeHintObject -> {
