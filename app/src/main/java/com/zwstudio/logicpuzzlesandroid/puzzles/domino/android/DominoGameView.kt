...
class DominoGameView : CellsGameView {
    private fun activity(): DominoGameActivity {
        return context as DominoGameActivity
    }

    private fun game(): DominoGame? {
...
//        canvas.drawColor(Color.BLACK);
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !game()!!.isSolved) {
            ...
            val move: DominoGameMove = object : DominoGameMove() {
                init {
                    p = Position(row, col)
