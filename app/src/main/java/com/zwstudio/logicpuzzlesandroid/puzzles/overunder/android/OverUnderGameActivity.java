...
public class OverUnderGameActivity extends GameGameActivity<OverUnderGame, OverUnderDocument, OverUnderGameMove, OverUnderGameState> {
    @Bean
    protected OverUnderDocument document;
    public OverUnderDocument doc() {return document;}

    protected OverUnderGameView gameView;
    protected View getGameView() {return gameView;}

    @AfterViews
    protected void init() {
        gameView = new OverUnderGameView(this);
        super.init();
    }
    ...
    protected void startGame() {
        ...
        game = new OverUnderGame(level.layout, this, doc());
        try {
            // restore game state
            for (MoveProgress rec : doc().moveProgress()) {
                OverUnderGameMove move = doc().loadMove(rec);
                game.setObject(move);
            }
