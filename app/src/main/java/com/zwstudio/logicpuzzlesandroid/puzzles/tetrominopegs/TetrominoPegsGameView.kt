...
class TetrominoPegsGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as TetrominoPegsGameActivity
...
//        canvas.drawColor(Color.BLACK);
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.action == MotionEvent.ACTION_DOWN && !game.isSolved) {
            ...
            val move = TetrominoPegsGameMove(Position(row, col), if (yOffset >= -offset && yOffset <= offset) 1 else 2)
