...
public class BoxItUpGameView extends CellsGameView {
    ...
    private BoxItUpGame game() {return activity().game;}
    ...
    public BoxItUpGameView(Context context) {
        super(context);
        init(null, 0);
    }

    public BoxItUpGameView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public BoxItUpGameView(Context context, AttributeSet attrs, int defStyle) {
...
//        canvas.drawColor(Color.BLACK);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN && !game().isSolved()) {
            ...
            BoxItUpGameMove move = new BoxItUpGameMove() {{
