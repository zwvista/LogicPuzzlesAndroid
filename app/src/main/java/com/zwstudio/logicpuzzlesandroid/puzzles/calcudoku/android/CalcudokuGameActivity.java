...
public class CalcudokuGameActivity extends GameGameActivity<CalcudokuGame, CalcudokuDocument, CalcudokuGameMove, CalcudokuGameState> {
    @Bean
    protected CalcudokuDocument document;
    public CalcudokuDocument doc() {return document;}

    protected CalcudokuGameView gameView;
    protected View getGameView() {return gameView;}

    @AfterViews
    protected void init() {
        gameView = new CalcudokuGameView(this);
        super.init();
    }
    ...
    protected void startGame() {
        ...
        game = new CalcudokuGame(level.layout, this, doc());
        try {
            // restore game state
            for (MoveProgress rec : doc().moveProgress()) {
                CalcudokuGameMove move = doc().loadMove(rec);
                game.setObject(move);
            }
