...
public class OrchardsGame extends CellsGame<OrchardsGame, OrchardsGameMove, OrchardsGameState> {
    ...
    public OrchardsGame(List<String> layout, GameInterface<OrchardsGame, OrchardsGameMove, OrchardsGameState> gi, GameDocumentInterface gdi) {
        super(gi, gdi);
        ...
        for (int r = 0; r < rows(); r++)
            for (int c = 0; c < cols(); c++) {
                Position p = new Position(r, c);
                for (int i = 0; i < 4; i++)
                    if (dots.get(p.add(OrchardsGame.offset2[i]), OrchardsGame.dirs[i]) != GridLineObject.Line)
                        g.connectNode(pos2node.get(p), pos2node.get(p.add(OrchardsGame.offset[i])));
            }
        while (!rng.isEmpty()) {
        ...
        OrchardsGameState state = new OrchardsGameState(this);
        states.add(state);
        levelInitilized(state);
    }

    private boolean changeObject(OrchardsGameMove move, F2<OrchardsGameState, OrchardsGameMove, Boolean> f) {
        if (canRedo()) {
            states.subList(stateIndex + 1, states.size()).clear();
            moves.subList(stateIndex, states.size()).clear();
        }
        OrchardsGameState state = cloner.deepClone(state());
    ...
    public OrchardsObject getObject(int row, int col) {
