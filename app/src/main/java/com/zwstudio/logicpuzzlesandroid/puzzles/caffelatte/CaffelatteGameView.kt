...
class CaffelatteGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
    private val activity get() = context as CaffelatteGameActivity
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> if (p != pLastMove) {
                val n = CaffelatteGame.offset.indexOfFirst { it == p - pLastMove!! }
                if (n != -1) {
                    val move = CaffelatteGameMove(pLastMove!!, n)
