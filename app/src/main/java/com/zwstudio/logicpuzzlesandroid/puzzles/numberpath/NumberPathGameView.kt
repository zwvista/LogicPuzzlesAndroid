...
class NumberPathGameView(context: Context, val soundManager: SoundManager) : CellsGameView(context) {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_UP -> {
                if (p == pLastDown) {
                    ...
                    val dx2 = abs(dx)
                    val dy2 = abs(dy)
                    val move = NumberPathGameMove(Position(row, col), if (dx2 <= dy2) if (dy > 0) 2 else 0 else if (dx > 0) 1 else 3)
                    game.setObject(move)
                }
                run {
                    pLastMove = null
