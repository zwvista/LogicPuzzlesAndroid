    ...
    public FourMeNotGame(List<String> layout, GameInterface<FourMeNotGame, FourMeNotGameMove, FourMeNotGameState> gi, GameDocumentInterface gdi) {
        ...
        for (int r = 0; r < rows(); r++)
            for (int c = 0; c < cols(); c++) {
                Position p = new Position(r, c);
                for (int i = 0; i < 4; i++)
                    if (dots.get(p.add(FourMeNotGame.offset2[i]), FourMeNotGame.dirs[i]) != GridLineObject.Line)
                        g.connectNode(pos2node.get(p), pos2node.get(p.add(FourMeNotGame.offset[i * 2])));
            }
        while (!rng.isEmpty()) {
